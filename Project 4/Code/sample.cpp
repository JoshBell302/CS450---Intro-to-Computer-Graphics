...
const char* WINDOWTITLE = { "Project 4 -- Lighting -- Josh Bell" };
...
Animate()
{
	...
	locationOfLight = locationOfLight + directionOfLight;

	if (locationOfLight > 10)
		directionOfLight = -.1;
	if (locationOfLight < -3)
		directionOfLight = .1;
...
Display()
{
	...
	// Torus - Blue - Shiny
	glPushMatrix();
	SetMaterial(0.0, 0.0, 1.0, 10);
	glShadeModel(GL_SMOOTH);
	glColor3f(0.0, 0.0, 1.0);
	glutSolidTorus(1., 3., 50, 50);
	glPopMatrix();

	// Flat Sphere - Red - Dull
	glPushMatrix();
	SetMaterial(1.0, 0.0, 0.0, 00);
	glShadeModel(GL_FLAT);
	glTranslatef(-3, -2, 5);
	...
	glDisable(GL_LIGHTING); // Make sure that above shapes are lightable and below are colored

	// GL_LIGHT0 Light Sphere - WHITE
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glTranslatef(2, 2, 2);
	...
	// GL_LIGHT2 Light Sphere - YELLOW
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glTranslatef(0, 0, locationOfLight);
	...
	SetSpotLight(GL_LIGHT1,	-2, 2, 2,	0, 0, -1,	1, 0, 0);

	SetSpotLight(GL_LIGHT2, 0, 0, locationOfLight,	0, 0, -1,	 1., 1., .0);
